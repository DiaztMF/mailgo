...
- When project marked finished/complete or deployment init requested, follow `C:\Users\advan\.config\opencode\production-readiness.md` (Docker multi-stage pinned + non-root, Husky gates, deploy.yml test→Trivy→GHCR→Coolify, dependabot weekly). `.env.example` keys only.
