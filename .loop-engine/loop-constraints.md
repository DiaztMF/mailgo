...
## Deployment
- Default deploy branch is `master`, never another branch. Vercel production branch must be `master`.

## Budget
- Budget is observability-only — all models are free, never block on token cap
- Do not switch to report-only for budget reasons unless human explicitly says so
